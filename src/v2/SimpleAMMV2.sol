...
import "../interfaces/ISimpleAmmV2.sol";
...
contract SimpleAMMV2 is ISimpleAMMV2, IMultiSigExecutor, ReentrancyGuardTransient, AccessControl, Pausable {
    ...
    /// @param newFee New fee value in basis points (1 = 0.01%)
    /// @dev Can only be called by accounts with OPERATOR_ROLE
    function setFee(uint256 newFee) external onlyRole(OPERATOR_ROLE) {
        if (newFee > FEE_DENOMINATOR) revert SimpleAMM__InvalidAmount(newFee, FEE_DENOMINATOR);
        uint256 oldFee = store.getFee();
        store.setFee(newFee);
        ...
        returns (uint256 shares)
    {
        ...
        // Cache storage reads and msg.value
        ...
        if (lpTokenAddr == address(0)) {
            ...
            // Calculate amounts using unchecked where safe
            unchecked {
        ...
        // Cache LP token contract to avoid multiple external calls
        ...
        mustInitialized
    {
        ...
        // Cache msg.value to avoid multiple MLOAD operations
        uint256 ethIn = msg.value;

        // Compute amounts in a single operation to save gas
        uint256 tokensBought = _getAmountOut(ethIn, ethReserve, tokenReserve);
        if (tokensBought < minTokensOut) revert SimpleAMM__InsufficientOutput(tokensBought, minTokensOut);

        uint256 priceImpact = _calculatePriceImpact(ethIn, tokensBought, ethReserve, tokenReserve);
        ...
        store.setPoolEthReservesWithAddress(tokenAddress, ethReserve + ethIn);

        // Interactions
        emit TokenSwap(tokenAddress, tokensBought, ethIn);
    ...
    function _getAmountOut(uint256 amountIn, uint256 reserveIn, uint256 reserveOut) private view returns (uint256) {
        ...
        uint256 fee = store.getFee();

        // Combine operations to reduce intermediate variables
